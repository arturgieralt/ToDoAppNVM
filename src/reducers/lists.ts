  ...
  if (listId === list.id) {
    ...
    if (updatedTasks[oldPosition] === taskId) {
      updatedTasks.splice(
        newPosition,
        0,
        updatedTasks.splice(oldPosition, 1)[0]
      );
    }
    return Object.assign({}, list, { orderedTasks: updatedTasks });
  }
...
const sortTasks = (allTasks: ITask[], currentDate: Moment) => (list: IList) => {
  ...
  const [outdated, uptodate] = tasksToOrder.reduce(
    makeArrayPartition(allTasks, currentDate),
    [[], []]
  );
...
const makeArrayPartition = (allTasks: ITask[], currentDate: Moment) => (
  [old, fresh]: string[][],
  id: string,
  index: number
) => {
...
export function listsReducer(state: IList[] = [], action: Action<string>) {
  switch (action.type) {
    case actionTypes.ADD_LIST:
      return [
        ...state,
        { ...(action as actionTypes.AddListAction<actionTypes.ADD_LIST>).list }
      ];
    case actionTypes.ADD_TASK:
      const task = {
        ...(action as actionTypes.AddTaskAction<actionTypes.ADD_TASK>).task
      };
    ...
    case actionTypes.CHANGE_TASK_ORDER:
      const {
        ...
        oldPosition,
        taskId
      } = action as actionTypes.ChangeTaskOrderAction<
        actionTypes.CHANGE_TASK_ORDER
      >;
      return state.map(
        changeTaskOrder(listId, newPosition, oldPosition, taskId)
      );
    case actionTypes.SORT_TASKS:
      const allTasks = [
        ...(action as actionTypes.SortTasksAction<actionTypes.SORT_TASKS>).tasks
      ];
      const { currentDate } = action as actionTypes.SortTasksAction<
